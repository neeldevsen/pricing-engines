...
Diagonals<S_t, K_t, r_t, sigma_t, T_t> generateDiffusionMatrix(const auto deltaS, const OptionDataBS<S_t, K_t, r_t, sigma_t, T_t>& data, const int M)
{
    ...
    for (int i {1}; i < M-1; ++i)
    {
        ...
        mass_diag.A_upper[i] = diff_diag.A_lower[i]
    }
    mass_diag.A_main[M-1] =  0.5 * deltaS * (data.volatility) * (data.volatility)  * (std::pow(((static_cast<double>(M) - 0.5), 2.0)));
    mass_diag.A_main[0] = 0.125 * deltaS * (data.volatility) * (data.volatility); // would review this line because I did i = 0 directly in the calculation
    return mass_diag;
}
...
Diagonals<S_t, K_t, r_t, sigma_t, T_t> generateConvectionMatrix(const auto deltaS, const OptionDataBS<S_t, K_t, r_t, sigma_t, T_t>& data, const int M)
{
    ...
    mass_diag.A_main[M-1] =  0.5 * deltaS * (data.volatility * data.volatility - data.rate)  * (static_cast<double>(M) - 0.5);
    mass_diag.A_main[0] = -0.25 * deltaS * (data.volatility * data.volatility - data.rate); // would review this line because I did i = 0 directly in the calculation
    conv_diag.A_upper[0] = 0.25 * deltaS * (data.volatility * data.volatility - data.rate);
    conv_diag.A_lower[0] = -conv_diag.A_lower[0]
    return mass_diag; // check this asap
}
