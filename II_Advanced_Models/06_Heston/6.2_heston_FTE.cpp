...
auto hestonFTECall(OptionDataHeston<S_t, K_t, r_t, T_t, v_t, kappa_t, theta_t, xi_t, rho_t>& data, int M, int N)
{
    ...
    for (int i {0}; i < M; ++ i)
    {
        ...
        for (int j {0}; j < N; ++j)
        {
            ...
            variance += data.kappa * (data.theta - std::max(variance,0)) * deltaT + data.xi * std::sqrt(std::max(variance, 0) * deltaT) * Zv;
        }
    }
